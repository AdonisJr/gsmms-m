import { View, Text, Button, TextInput, ScrollView, TouchableOpacity, ActivityIndicator } from 'react-native';
...
import { editRequest, fetchAvailableServices } from '../../services/apiServices';
import { useUserStore } from '../../store/userStore';

export default function EditRequestedServices({ navigation, route }) {
    ...
    const [formData, setFormData] = useState({
        ...
        number_of_personnel: selected.number_of_personnel,
        classification: selected.classification,
        expected_start_date: selected.expected_start_date,
        expected_end_date: selected.expected_end_date,
        other: selected.other,
        status: selected.status
    });
    // console.log(typeof selected.expected_end_date)
    ...
    const handleRequest = async () => {
        ...
        try {
            setLoading(true);
            const response = await editRequest(formData);
    ...
    return (
        <View className="flex-1 bg-white">
            <View className="mt-4 ms-4 mb-2 fixed">
            ...
            <ScrollView className="p-4">
                ...
                <View className="mb-4 bg-white border border-gray-300 rounded-lg p-4 shadow-md">
                    ...
                    <TextInput
                        ...
                        onChangeText={(value) => setFormData({ ...formData, other: value })}
                        className="border border-gray-300 p-2 mt-2 rounded"
                    />
                </View>
                ...
                <TouchableOpacity
                    ...
                    className="mb-2 p-3"
                >
                    <Text>{new Date(formData.expected_start_date).toLocaleDateString()}</Text>
                </TouchableOpacity>

                {/* Date picker modal */}
                {showDatePicker && (
                    <DateTimePicker
                        value={new Date(formData.expected_start_date)}
                        mode="date"
                        display="default"
                ...
                <TouchableOpacity
                    ...
                    className="mb-2 p-3"
                >
                    <Text>{new Date(formData.expected_end_date).toLocaleDateString()}</Text>
                </TouchableOpacity>

                {/* Date picker modal */}
                {showDatePicker2 && (
                    <DateTimePicker
                        value={new Date(formData.expected_end_date)}
                        mode="date"
                        display="default"
